...
generate_from_bootstrap() {
    ...
    # service.yml (which goes to services/) and the .devcontainer dir
    # (handled separately below) is treated as an app source stub. This
    # ships things like main.go / go.mod / .air.toml / src/index.js so
    # the build context isn't empty — `./devstack.sh start` succeeds
    # before the user has written any application code. Stubs are
    # trivially overwritten by the user's real application.
    _copy_app_source_stubs() {
        ...
        for entry in "${src_dir}"/* "${src_dir}"/.[!.]*; do
            ...
            name=$(basename "${entry}")
            case "${name}" in
                service.yml|Dockerfile|.devcontainer) continue ;;
            esac
            cp -r "${entry}" "${dst_dir}/${name}"
            log "  Copied app stub: ${name}" >&2
        done
    }
    _copy_app_source_stubs \
    ...
    if [ "${frontend_type}" != "none" ]; then
        ...
        # Frontend source stubs: same pattern as app — copy any extra
        # source files (nuxt.config.ts, app.vue, src/, etc.) so the
        # frontend container has something to render before the user
        # writes their UI. Skip Dockerfile/service.yml; package.json is
        # generated separately further down (per-framework).
        _copy_frontend_source_stubs() {
            local src_dir="$1"
            local dst_dir="$2"
            [ -d "${src_dir}" ] || return 0
            local entry name
            for entry in "${src_dir}"/* "${src_dir}"/.[!.]*; do
                ...
                name=$(basename "${entry}")
                case "${name}" in
                    service.yml|Dockerfile|package.json) continue ;;
                esac
                cp -r "${entry}" "${dst_dir}/${name}"
                log "  Copied frontend stub: ${name}" >&2
            done
        }
        _copy_frontend_source_stubs \
            "${DEVSTACK_DIR}/templates/frontends/${frontend_type}" \
            "${dest}/frontend"
    fi
...
SPEC
    ...
    # the user hasn't already provided one. Without a real file at
    # ${APP_SOURCE}/${PRISM_SPEC_PATH}, Docker will helpfully create a
...
# Minimal OpenAPI 3 spec shipped by dev-strap so the Prism mock server
# has something to serve before the user has authored a real spec.
# Overwrite this with the real backend spec — Prism will hot-reload via
# `./devstack.sh restart`.
openapi: 3.0.3
info:
  title: DevStack stub API
  version: 0.0.0
  description: |
    Placeholder spec generated by dev-strap. Replace with your real
    OpenAPI document.
paths:
  /health:
    get:
      summary: Stub health endpoint
      responses:
        '200':
          description: OK
          content:
